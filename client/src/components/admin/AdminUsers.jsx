...
//     try {
//       setLoading(true)
//       const { data } = await api.get('/admin/users')  // backend lists ALL
...
//     try {
//       // Requires a DELETE endpoint on backend, e.g. DELETE /admin/users/<id>
//       await api.delete(`/admin/users/${id}`)
//       setRows(prev => prev.filter(r => r.id !== id))
//     } catch (err) {
//       setError(err?.response?.data?.msg || 'Delete route not available on server')
//     }
//   }
...
//       <h2 className="mb-6 text-2xl font-bold text-gray-900">Admin · Users</h2>
...
//           <button onClick={load} className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
//             Refresh
//           </button>
//         </div>
...
//                     <td className="px-6 py-4"><span className={badge(u.role)}>{u.role}</span></td>
...
/* ---------- Shared inline layout (sidebar + header) ---------- */
      ...
      <div className="flex">
        ...
        <aside className={`fixed inset-y-0 z-40 w-64 transform bg-white p-4 shadow-lg ring-1 ring-gray-200 transition-transform md:static md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          ...
          <div className="mt-6 border-t pt-4">
            <button onClick={logout} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-left text-sm font-medium text-gray-700 hover:bg-gray-50">
              Logout
            </button>
          </div>
        </aside>
        ...
        <div className="flex min-h-screen w-full flex-col md:ml-64">
          <header className="sticky top-0 z-30 flex items-center justify-between border-b bg-white px-4 py-3">
            ...
            <button className="rounded-lg border px-3 py-2 text-sm font-medium md:hidden" onClick={()=>setOpen(v=>!v)}>
              {open ? 'Close' : 'Menu'}
            </button>
          </header>
...
/* ---------- Users page content (list all + create) ---------- */
function UsersContent() {
  ...
  async function handleDelete(id, name) {
    if (!window.confirm(`Delete "${name}"? This cannot be undone.`)) return
    try {
      await api.delete(`/admin/users/${id}`) // requires DELETE endpoint on backend
      setRows(prev => prev.filter(r => r.id !== id))
    } catch (err) {
      setError(err?.response?.data?.msg || 'Delete route not available on server')
    }
